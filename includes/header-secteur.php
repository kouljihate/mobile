...
<html> 
	...
	{
		 $('#dataTable').fadeOut("slow").load('includes/dataTable.php').fadeIn("slow");
	}, 10000);
	*/																			
	   ...
	   var refreshId = setInterval(function() {
		...
		$('#live-secteur').mouseover(function() {
			hovering = true;
		});
		
		$('#live-secteur').mouseout(function() {
			hovering = false;
		...
		function posts() {
		 pfunc = $.ajax({
				 url: 'includes/secteur_live_list.php',
				 success: function(data) {
								onResponse = function() {
									if(hovering) return;
									$('#live-secteur').html(data);
									$('#live-secteur').fadeIn(2000);
  ...
  <!-- end splash screen -->
	<div id="menu">		
		<ul>
            ...
            <li class="active"><a href="" class="contentLink">Secteurs<span></span></a></li>
			<li><a href="course-live.php" rel="external" class="contentLink">Mes courses<span></span></a></li>
